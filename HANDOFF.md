...
> - **Music player — "silent after long idle" fixed permanently (2026-06-19):** symptom = after the tab sat idle (~30 min) the UI shows *playing* and the visualizer animates but there's **no sound**. Cause: after a long background the AudioContext clock keeps ticking while its output is bound to a **dead/stale device**, so every probe (incl. the frozen-clock watchdog) reads "healthy" and the old code merely `resume()`d it. Fix in `MusicPlayer.tsx`: on returning after a **>20s** hide while playing, **rebuild** the pipeline onto the current device (fresh `<audio>` + context) instead of resuming (short tab-switches still just resume); and the rebuild's restore path refuses to claim "playing" if the new context can't reach `running` without a user gesture — it shows **paused** so one click re-plays audibly, never the silent-playing state. Earlier self-healing (GainNode volume, frozen-clock watchdog, devicechange rebuild) all still in place.
...
- **Music silent after long idle** (UI "playing" + visualizer moving, **no sound**): the AudioContext's clock keeps ticking on a dead/stale output device, so every probe — including the frozen-clock watchdog — reads healthy and a plain `resume()` never restores sound. **Fix:** on tab-return after a **>20s** hide while playing, *rebuild* the pipeline (fresh element + context, current device) instead of resuming; never show "playing" if the rebuilt context can't reach `running`. (Details in the 2026-06-19 music bullet up top.)
...
- **Finale email** — when a canvas reveals, email a web-friendly artwork to a collected opt-in list. **Deferred to Canvas Nº 002** (Mickey's call 2026-06-18, reaffirmed 2026-06-19): Nº 001 is ~2% full and closes 21 June, so the finale's "look what hundreds of strangers made" payoff is weak, and standing up a fresh send pipeline mid-judging isn't worth it. **Full design + build plan: `FINALE_EMAIL_PLAN.md`.** Sender reality (corrects an earlier note): Resend is *already* wired for Supabase Auth SMTP and **ekam.ink is already domain-verified**, so the only new infra is a `RESEND_API_KEY` in env + direct Resend API calls — no new account, no domain wait.
