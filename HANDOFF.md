...
> - **Deadlines (2026-06-12, migration 0008):** `canvases.closes_at` = 17 June 2026 11:59pm IST. After it: claims + submits return "closed" (typed results, not throws — server action errors are masked in prod), the finale reveal triggers at partial fill, and the wall composite renders blanks as paper (`createRealWall` `paperBlanks` arg). **48h claim window**: set at claim, RESET on every submission (returned tiles stay yours while you keep trying); 12h warning notification (deduped via `expiry_warned_at`); lapse → tile reopens clean + "expired" notification + log row. Enforced by a lazy `sweepClaimWindows()` in `after()` on /canvas (5 min throttle) + daily cron backstop in `vercel.json`. Pre-0008 claims (NULL `claim_expires_at`) never expire. All writes fall back without the 0008 columns so deploy order doesn't matter. Countdowns: landing hero, canvas sidebar, owner's tile panel (`components/Countdown.tsx`).
