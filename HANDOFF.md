...
> - **Finale (2026-06-12):** `lib/stitch.ts` composes the whole wall client side (published art, blanks as paper, 12-way loader, `crossOrigin` so the PNG export never taints; 384px/tile → 9216²; print res stays an offline job). Admin → "the artwork" tab previews the growing artwork any day (thumb stitch + caption + hi res download). When all 576 publish, `/canvas` reveals: one-time fullscreen confetti celebration (localStorage `ekam.finale.seen`), seamless artwork view (MosaicCanvas `seamless` prop kills grout), "The artwork / The tiles" toggle pill, serif caption "Made by N people, date to date", Download + Share. `/canvas?finale=1` previews the whole reveal at current fill, admin cookie only (celebration replays there, flag not persisted). Music player UI hides during celebration/art mode via `body[data-panel]`.
