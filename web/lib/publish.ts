...
export async function publishTile(db: ReturnType<typeof supabaseAdmin>, id: string, via: "admin" | "ai"): Promise<void> {
  const { data: tile } = await db.from("tiles").select("pending_image_path, pending_story, artist_email, artist_user_id, x, y, review_requested_at").eq("id", id).maybeSingle()
    .then((r) => r.error ? db.from("tiles").select("pending_image_path, pending_story, artist_email, artist_user_id, x, y").eq("id", id).maybeSingle() : r) as { data: { pending_image_path: string | null; pending_story: string | null; artist_email: string | null; artist_user_id: string | null; x: number; y: number; review_requested_at?: string | null } | null };
  if (!tile) return;
  ...
  await notify(db, { email: tile.artist_email, userId: tile.artist_user_id },
...
export async function aiRejectTile(db: ReturnType<typeof supabaseAdmin>, id: string, reason: string): Promise<void> {
  const { data: tile } = await db.from("tiles").select("status, pending_image_path, artist_email, artist_user_id, x, y").eq("id", id).maybeSingle();
  if (!tile) return;
  ...
  await notify(db, { email: tile.artist_email, userId: tile.artist_user_id }, "returned",
