...
const labelStyle: React.CSSProperties = {
  fontFamily: "var(--font-inter), sans-serif", fontSize: 14, fontWeight: 500, color: "var(--color-text-muted)",
};
const btn: React.CSSProperties = {
  fontFamily: "var(--font-inter), sans-serif", fontSize: 16, fontWeight: 500, color: "var(--color-text-inverse)",
...
export default function SignIn() {
    ...
    return (
      <form onSubmit={verify} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <button type="button" onClick={() => { setStep("email"); setCode(""); setErr(""); }}
          style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "var(--color-text-muted)", background: "none", border: "none", cursor: "pointer", alignSelf: "flex-start" }}>
  ...
  return (
    <form onSubmit={sendCode} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <p style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "var(--color-text-muted)", margin: 0 }}>no password — we&apos;ll email you a 6-digit code.</p>
    </form>
  );
}
