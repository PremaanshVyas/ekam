...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  // next/og can't reliably fetch remote <img src> — inline the PNG as a data URI.
  let imgData: string | null = null;
  if (img) { try { const r = await fetch(img, { signal: AbortSignal.timeout(6000) }); if (r.ok) imgData = `data:image/png;base64,${Buffer.from(await r.arrayBuffer()).toString("base64")}`; } catch {} }
      ...
      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", gap: 64, padding: "0 80px", background: "#16110d", color: "#efe9e1", fontFamily: "Inter" }}>
        <div style={{ display: "flex", width: 460, height: 460, borderRadius: 14, overflow: "hidden", border: "1px solid rgba(239,233,225,0.12)", background: "#f4eee2", flex: "none" }}>
          {imgData ? (
            ...
            <img src={imgData} width={460} height={460} style={{ objectFit: "cover" }} />
          ) : (
