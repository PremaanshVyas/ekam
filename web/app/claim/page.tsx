      ...
      <div style={{ width: 440, maxWidth: "100%", background: "var(--color-bg-elevated)", borderRadius: 16, padding: 32, display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 4px 16px rgba(32,32,29,.16)" }}>
        ...
        {!user ? (
          ...
          <form action={claimTile} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <p style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "var(--color-text-muted)", margin: 0 }}>signed in as {user.email}</p>
            ...
            <p style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "var(--color-text-muted)", margin: 0 }}>one tile per person · you have 24h to paint it</p>
          </form>
        )}
      </div>
    </main>
